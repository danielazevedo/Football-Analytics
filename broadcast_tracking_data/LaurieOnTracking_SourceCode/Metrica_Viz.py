...
def my_plot_events( events, figax=None, field_dimen = (106.0,68), indicators = ['Marker','Arrow'], colors=['r'], marker_style = [], alpha = [], annotate=False):
    ...
    for i,row in events.iterrows():
        ...
        if 'Marker' in indicators:
            ax.plot(  row['Start X'], row['Start Y'], marker_style[counter], alpha=alpha[counter], color=colors[counter])
        if 'Arrow' in indicators:
            ax.annotate("", xy=row[['End X','End Y']], xytext=row[['Start X','Start Y']], alpha=alpha[counter], arrowprops=dict(alpha=alpha[counter],width=0.5,headlength=4.0,headwidth=4.0,color=colors[counter]),annotation_clip=False)
        if annotate:
            textstring = row['From']
            ax.text( row['Start X'], row['Start Y'], textstring, fontsize=10, color=colors[counter])
