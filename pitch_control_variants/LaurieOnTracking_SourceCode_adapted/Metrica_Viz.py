...
def my_plot_events( events, figax=None, field_dimen = (106.0,68), indicators = ['Marker','Arrow'], color='r', marker_style = [], alpha = [], annotate=False):
    """ my_plot_events( events )
    ...
    for i,row in events.iterrows():
        ...
        if 'Marker' in indicators:
            ax.plot(  row['Start X'], row['Start Y'], color+marker_style[counter], alpha=alpha[counter] )
        if 'Arrow' in indicators:
            ax.annotate("", xy=row[['End X','End Y']], xytext=row[['Start X','Start Y']], alpha=alpha[counter], arrowprops=dict(alpha=alpha[counter],width=0.5,headlength=4.0,headwidth=4.0,color=color),annotation_clip=False)
        if annotate:
            textstring =row['From']
...
def plot_events( events, figax=None, field_dimen = (106.0,68), indicators = ['Marker','Arrow'], color='r', marker_style = 'o', alpha = 0.5, annotate=False, success=False):
    """ plot_events( events )
