...
def generate_pitch_control_for_event(new_events, event_id, events, tracking_home, tracking_away, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True, scenario='normal'):
    ...
    for i in range( len(ygrid) ):
        for j in range( len(xgrid) ):
            ...
            PPCFa[i,j],PPCFd[i,j] = calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params, scenario, new_events)
...
def get_players_radious(player_position, players_opp, r, p):
    # player radious
    #r = 15
    
    # threshold for number of players around
    #p = 4
    
    distances = np.array([np.linalg.norm(player_position - b.position) for b in players_opp])
    
    if sum(distances < r) >= p:
        return True

    return False


def get_norm_passes(events, pos_x, pos_y, start_x, start_y):
    x = 40
    relevant_events = events[(events['End X'] < pos_x + x) & (pos_x - x < events['End X'])\
                             & (events['End Y'] < pos_y + x) & (pos_y - x < events['End Y'])]
    
    relevant_events = relevant_events[(relevant_events['Start X'] < start_x + x) & (start_x - x < relevant_events['Start X'])\
                             & (relevant_events['Start Y'] < start_y + x) & (start_y - x < relevant_events['Start Y'])]
    
    diff_passes = np.sum(relevant_events['status_actions'] == 1) - np.sum(relevant_events['status_actions'] == 0)
    
    return round(diff_passes/len(relevant_events), 3)


def calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params, scenario, events):
    ...
    if tau_min_att-max(ball_travel_time,tau_min_def) >= params['time_to_control_def']:
        ...
        while 1 - ptot > params['model_converge_tol'] and i < dT_array.size: 
            ...
            if scenario == 'side':
                ...
                x_dist = ((max(target_position[0], ball_start_pos[0]) - np.abs(target_position[0] - ball_start_pos[0]))/106)**2
                dist = (np.abs(target_position[1])/34)**3
                
                PPCFatt[i] += alpha * dist + beta * x_dist
                PPCFdef[i] -= alpha * dist + beta * x_dist

            elif scenario == 'vertical':
                theta = 0.5
                
                if target_position[0] - ball_start_pos[0] > 0:
                    dist = (target_position[0] - ball_start_pos[0])/106
                    PPCFatt[i] += theta * dist
                    PPCFdef[i] -= theta * dist
                
            elif scenario == 'possession':
                penalization = 0.25
                if get_players_radious(target_position, defending_players, 15, 2):
                    PPCFatt[i] -= penalization
                    PPCFdef[i] += penalization
            
            elif scenario == 'history':
                gamma = 0.55
                perc = get_norm_passes(events.copy(), target_position[0], target_position[1], ball_start_pos[0], ball_start_pos[1])
                
                PPCFatt[i] += gamma * perc
                PPCFdef[i] -= gamma * perc
